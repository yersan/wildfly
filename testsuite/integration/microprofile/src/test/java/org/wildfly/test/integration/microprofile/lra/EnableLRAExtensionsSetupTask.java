    ...
    public EnableLRAExtensionsSetupTask() {
        super(createContainerConfigurationBuilder()
            .setupScript(createScriptBuilder()
                ...
                .endBatch()
                .startBatch()
                .add("/subsystem=%s:write-attribute(name=url, value=http://%s:8080/lra-coordinator/lra-coordinator)",
                    SUBSYSTEM_LRA_PARTICIPANT, System.getProperties().containsKey("ipv6") ? "[::1]" : "127.0.0.1")
                .endBatch()
                .build()
            )
